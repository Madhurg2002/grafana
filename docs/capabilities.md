...
| Share links: audience (anyone-link / email allow-list / org members) × right (view/edit), revoke, public snapshot view — **multiple links per tenant coexist** (a view link and an edit link live side by side; revoking one never touches the others, migration 013); snapshots embed the tenant's composed pages + widgets with server-rendered values (`pages[]`, tabbed in ShareView; falls back to the built-in layout for pre-pages links) | `backend/src/routes/share.ts`, `backend/src/db/migrations/013-share-links-multi.ts`, `ShareDialog.tsx`, `ShareView.tsx` |
