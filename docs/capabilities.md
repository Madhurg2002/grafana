...
| Share links: audience (anyone-link / email allow-list / org members) × right (view/edit), revoke, public snapshot view — snapshots embed the tenant's composed pages + widgets with server-rendered values (`pages[]`, tabbed in ShareView; falls back to the built-in layout for pre-pages links) | `backend/src/routes/share.ts`, `ShareDialog.tsx`, `ShareView.tsx` |
